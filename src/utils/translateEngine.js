...
export async function createTranslateEngine(location) {
  return {
    ...
    getPotentialTranslators: async () => {
      const provider = new TranslatorProvider();
...
class TranslatorProvider {
  ...
  async _ensure() {
    ...
    const TranslatorsManifest = await (await fetch(manifestUrl)).json();
  ...
  */
  async getWebTranslatorsForLocation(URI, rootURI) {
    const matches = [];
    const translators = await this.getAllForType("web");
    console.log(
      "[TranslatorProvider] Looking for web translators for %s among %d translators",
      URI,
      translators.length,
    );
    ...
    console.log(
      "[TranslatorProvider] URL %s matched %d translator candidate(s): %o",
      URI,
      matches.length,
      matches.map((translator) => translator.label),
    );
    return [matches];
  }
  ...
  */
  async getCodeForTranslator(translator) {
    ...
    if (translator.file?.path) {
      ...
      console.log(
        "[TranslatorProvider] Loaded translator code for %s from %s (%d chars)",
        translator.label,
        url,
        code.length,
      );
